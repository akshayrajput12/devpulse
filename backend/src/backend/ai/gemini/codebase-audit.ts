...
async function auditFileChunk(params: {
  ...
  for (let attempt = 0; attempt < maxRetries; attempt += 1) {
    ...
    if (res.status !== 429 || attempt === maxRetries - 1) {
      throw new Error(`Gemini chunk audit API error ${res.status}: ${body.slice(0, 500)}`);
    }

    const delay = 3000 * Math.pow(2, attempt);
    logger.warn("gemini.chunk_rate_limited_retry", {
      requestId: params.requestId,
      reviewId: params.reviewId,
      attempt: attempt + 1,
      delayMs: delay,
  ...
  throw new Error("Gemini chunk audit failed after retries");
}
...
export async function callCodebaseAuditAI(params: {
  ...
  if (files.length <= 15 || !parallelEnabled) {
    logger.info("gemini.codebase_audit.single_step", {
      requestId: params.requestId,
      reviewId: params.reviewId,
    ...
    for (let attempt = 0; attempt < maxRetries; attempt += 1) {
      ...
      const delay = 3000 * Math.pow(2, attempt);
      logger.warn("gemini.single_step_rate_limited_retry", {
        requestId: params.requestId,
        reviewId: params.reviewId,
        attempt: attempt + 1,
        delayMs: delay,
    ...
    throw new Error("Gemini codebase audit failed after retries");
  }

  logger.info("gemini.codebase_audit.parallel_chunk_start", {
    requestId: params.requestId,
    reviewId: params.reviewId,
  ...
  logger.info("gemini.codebase_audit.parallel_chunk_complete", {
    requestId: params.requestId,
    reviewId: params.reviewId,
...
${JSON.stringify(mergedFindings, null, 2)}`;
  ...
  for (let attempt = 0; attempt < maxRetries; attempt += 1) {
    ...
    if (res.ok) {
      ...
      logger.info("gemini.codebase_audit.synthesis_complete", {
        requestId: params.requestId,
        reviewId: params.reviewId,
    ...
    const body = await res.text();
    if (res.status !== 429 || attempt === maxRetries - 1) {
      throw new Error(`Gemini codebase synthesis API error ${res.status}: ${body.slice(0, 500)}`);
    }

    const delay = 3000 * Math.pow(2, attempt);
    logger.warn("gemini.synthesis_rate_limited_retry", {
      requestId: params.requestId,
      reviewId: params.reviewId,
      attempt: attempt + 1,
      delayMs: delay,
  ...
  throw new Error("Gemini codebase audit synthesis failed after retries");
}
