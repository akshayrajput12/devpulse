import { callGeminiRaw, parseJsonResponse, readGeminiText } from "./client.js";
import { getGeminiApiKey, REVIEW_MODEL, TRIAGE_MODEL } from "./models.js";
...
async function triageFolder(requestId: string, structure: string, apiKey: string) {
  try {
    ...
    logger.info("gemini.folder_triage_usage", { requestId, model: TRIAGE_MODEL, usage });
    return text.trim().toUpperCase().includes("TRIVIAL") ? "TRIVIAL" : "REVIEW";
  } catch (error) {
    logger.warn("gemini.folder_triage_failed_open", { requestId, error });
    return "REVIEW";
  }
}
...
export async function callFolderAnalysisAI(params: {
  ...
  if (triage === "TRIVIAL") {
    logger.info("gemini.folder_analysis.trivial_early_return", {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    if (res.ok) {
      ...
      logger.info("gemini.folder_analysis_usage", {
        requestId: params.requestId,
        model: REVIEW_MODEL,
        usage,
    ...
    if (res.status !== 429 || attempt === 2) {
      throw new Error(`Gemini API folder analysis error ${res.status}: ${body.slice(0, 500)}`);
    }
    await new Promise(resolve => setTimeout(resolve, 3000 * Math.pow(2, attempt)));
  }

  throw new Error("Folder analysis failed after retries on Gemini");
}
