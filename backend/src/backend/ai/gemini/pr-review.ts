...
import { getGeminiApiKey, selectModel, TRIAGE_MODEL } from "./models.js";
...
async function triageFileDiff(requestId: string, reviewId: string, fileDiff: string, apiKey: string) {
  try {
    ...
    logger.info("gemini.triage_usage", { requestId, reviewId, model: TRIAGE_MODEL, usage });
    return text.trim().toUpperCase().includes("TRIVIAL") ? "TRIVIAL" : "REVIEW";
  } catch (error) {
    logger.warn("gemini.triage_failed_open", { requestId, reviewId, error });
    return "REVIEW";
  }
}
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (res.status !== 429 || attempt === maxRetries - 1) {
        logger.warn("gemini.file_audit_error", { filePath, status: res.status, body: body.slice(0, 300) });
        return { findings: [], usage: { promptTokenCount, candidatesTokenCount, totalTokenCount } };
      }
    } catch (err) {
      logger.warn("gemini.file_audit_exception", { filePath, error: String(err) });
      if (attempt === maxRetries - 1) {
        return { findings: [], usage: { promptTokenCount, candidatesTokenCount, totalTokenCount } };
      }
    }
    const delay = 2000 * Math.pow(2, attempt);
    logger.warn("gemini.file_audit_rate_limited_retry", {
      requestId,
      reviewId,
      filePath,
      attempt: attempt + 1,
...
export async function callPrReviewAI(params: {
  ...
  for (const [path, chunk] of activeEntries) {
    ...
    // Add artificial delay between triage calls to respect Gemini RPM rate limit
    await new Promise(resolve => setTimeout(resolve, 1000));
  }

  logger.info("gemini.triage_complete", {
  ...
  for (const { path, chunk } of filesToReview) {
    ...
    // Add artificial delay between files to respect Gemini RPM rate limit
    await new Promise(resolve => setTimeout(resolve, 1000));
  }
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      const body = await res.text();
      if (res.status !== 429 || attempt === maxRetries - 1) {
        throw new Error(`Gemini synthesis failed with status ${res.status}: ${body.slice(0, 300)}`);
      }
    } catch (err) {
      if (attempt === maxRetries - 1) throw err;
    }
    const delay = 2000 * Math.pow(2, attempt);
    logger.warn("gemini.pr_synthesis_rate_limited_retry", {
      requestId: params.requestId,
      reviewId: params.reviewId,
      attempt: attempt + 1,
      delayMs: delay,
