...
async function auditApiChunk(params: {
  ...
  for (let attempt = 0; attempt < maxRetries; attempt += 1) {
    ...
    if (res.status !== 429 || attempt === maxRetries - 1) {
      throw new Error(`OpenAI API analyser chunk error ${res.status}: ${body.slice(0, 500)}`);
    }

    const delay = 3000 * Math.pow(2, attempt);
    logger.warn("openai.api_chunk_rate_limited_retry", {
      requestId: params.requestId,
      reviewId: params.reviewId,
      attempt: attempt + 1,
      delayMs: delay,
  ...
  throw new Error("OpenAI API analyser chunk failed after retries");
}
...
export async function callApiAnalysisAI(params: {
  ...
  logger.info("openai.api_analysis.start", {
    requestId: params.requestId,
    reviewId: params.reviewId,
  ...
  if (files.length <= 15 || !parallelEnabled) {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt += 1) {
      ...
      const body = await res.text();
      if (res.status !== 429 || attempt === maxRetries - 1) {
        throw new Error(`OpenAI API analyser error ${res.status}: ${body.slice(0, 500)}`);
      }

      const delay = 3000 * Math.pow(2, attempt);
      logger.warn("openai.api_single_step_rate_limited_retry", {
        requestId: params.requestId,
        reviewId: params.reviewId,
        attempt: attempt + 1,
        delayMs: delay,
    ...
    throw new Error("OpenAI API analyser failed after retries");
  }
  ...
  logger.info("openai.api_analysis.parallel_chunk_complete", {
    requestId: params.requestId,
    reviewId: params.reviewId,
...
${JSON.stringify(mergedFindings, null, 2)}`;
  ...
  for (let attempt = 0; attempt < maxRetries; attempt += 1) {
    ...
    if (res.ok) {
      ...
      logger.info("openai.api_analysis.synthesis_complete", {
        requestId: params.requestId,
        reviewId: params.reviewId,
    ...
    const body = await res.text();
    if (res.status !== 429 || attempt === maxRetries - 1) {
      throw new Error(`OpenAI API analysis synthesis error ${res.status}: ${body.slice(0, 500)}`);
    }

    const delay = 3000 * Math.pow(2, attempt);
    logger.warn("openai.api_synthesis_rate_limited_retry", {
      requestId: params.requestId,
      reviewId: params.reviewId,
      attempt: attempt + 1,
      delayMs: delay,
  ...
  throw new Error("OpenAI API analyser synthesis failed after retries");
}
